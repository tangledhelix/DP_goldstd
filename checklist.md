...
"The gold standard." by White, Horace
...
* [x] [En dash](https://www.pgdp.net/wiki/En-dash):
  ...
  * Ignore the first regex (em-dash) and do these:
    ...
    * Negative numbers `–14º` (uses MINUS SIGN)
...
* [x] Use `File → Save a Copy As` to make `goldstd.html`
...
* [x] Open `goldstd.html` that was saved previously.
...
There exists another dash (HORIZONTAL BAR, U+2015) which one PM/PP prefers to EM DASH (using two bars for one EM DASH), based on appearance in text version. I opted not to use this in favor of using the EM DASH character in HTML and two hyphens in text.
...
* [x] If {super,sub}scripts were used, convert to `<sup>` or `<sub>` (NOTE: does Guiguts do this itself? Seems to for subs.)
* [x] Semantic fixup for italics
  * Use the Regex Library tool and my **custom** `my_italic_semantic.json`.
...
* [x] Handle any `<abbr>` for fractions that's needed?
  * Reminder: this regex: `([¹²³⁴⁵⁶⁷⁸⁹⁰]+⁄[₁₂₃₄₅₆₇₈₉₀]+|[¼½¾⅓⅔⅕⅖⅗⅘⅙⅚⅛⅜⅝⅞⅐⅑⅒↉])` may be of use to locate fractions once converted.

### Table notes

#### Wide tables and scrolling
Surround your table in a div using `overflow-x` and it will become scrollable in the browser. (YMMV in e-readers.)
...
I used this technique in `catholichistory`.

#### Large brackets or braces grouping table cells
Tables using wide-span braces or brackets are a pain. An image adjusted for size in CSS probably works the best. See `eugenics` for examples. That project used both horizontal and vertical curly braces in tables, using images:

* [images/bottom-brace.png](https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/bottom-brace.png)
* [images/left-brace.png](https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/left-brace.png)
* [images/right-brace.png](https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/right-brace.png)
* [images/top-brace.png](https://raw.githubusercontent.com/tangledhelix/DP_eugenics/refs/heads/main/images/top-brace.png)

The CSS will need tweaking for each specific project's needs.

```css
/* left/right braces */
.lr-brace { height: 2.25em; max-height: 2.25em; }
/* top/bottom braces */
.tb-brace { width: 3em; max-width: 3em; }

.c014 { vertical-align: middle; text-align: right; padding-left: .5em;
        padding-right: .5em; }
.c015 { vertical-align: middle; text-align: center; padding-left: .5em;
        padding-right: .5em; }

```

```html
<!-- left brace spanning 2 rows -->
<td class="c014" rowspan="2"> <img class="lr-brace" alt="left curly bracket" src="images/left-brace.png"></td>

<!-- top brace spanning 2 columns -->
<td class="c015" colspan="2"><img class="tb-brace" alt="top curly bracket" src="images/top-brace.png"></td>
```

### Drop caps
Drop-cap CSS is in my custom header; markup looks like:

```html
<p class="drop-cap"><span class="smcap uppercase">The</span>
```

* [x] Handle any drop-caps
...
**NOTE:** Use `<span id=`, not `<a id=`, for the target of the below links.

```html
<ul>
<li>
<abbr title="page">p.</abbr> 123: changed “foo” to “fool”
...
* [x] Go to [project page](https://www.pgdp.net/c/project.php?id=projectID624c94ea72f43), select SR time period, upload `goldstd-sr.zip`
...
* [x] Add **anonymized** files to git, e.g. `goldstd-smoothread01.txt`
...
* [x] Enter copyright clearance (get from DP [project page](https://www.pgdp.net/c/project.php?id=projectID624c94ea72f43))
...
* [x] Credits line
  * Paste in from DP [project page](https://www.pgdp.net/c/project.php?id=projectID624c94ea72f43)
